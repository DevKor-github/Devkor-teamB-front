...
import {Comment, Attachment} from '@src/Types';
...
import {earnPoints, givePoints, RewardType} from '../Store/StoreHandler';
import {fetchStudentImage} from '@src/data/studentApi';
import {deleteComment} from './PostAPI';
import {getToken, getUserNickname} from '@src/data/authStorage';
  ...
  return (
    <KeyboardAvoidingView
      ...
      <View style={commentStyles.inner}>
        <View style={[commentStyles.textfield, {height: inputHeight + 25}]}>
          <TextInput
            style={{
              flex: 1,
...
export function CommentContainer({
  ...
  isMyPost: boolean;
  notChosen: boolean;
  onChoose: (commentId: number, isChosen: boolean) => void;
}) {
    ...
    Alert.alert('포인트를 사용하시겠습니까?', `현재 보유 포인트: ${point}`, [
      ...
      },
      {
        ...
        onPress: () => {
          ...
          navigation.navigate('StoreNavigator', {screen: 'StoreScreen'});
        },
      },
    ]);
  };
      ...
      <View style={style.commentArea}>
        ...
        {isBlurVisible && (
          <View style={style.commentArea_Blur}>
            ...
            <TouchableOpacity
              onPress={() => toggleBlur(point)}
              style={style.button_Blur}>
