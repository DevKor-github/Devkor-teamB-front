...
const MyCommentScreen = () => {
  ...
  const fetchMyComment = async () => {
    ...
    try {
      ...
      //   console.log(item);
      // });
      const formattedComments: MyComment[] = detailedComments.map(
        (item: any) => ({
          comment: {
            commentId: item.id,
  ...
  const handleNavigate = async (comment: MyComment) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/posts/${comment.comment.postId}/`,
        {
          headers: {
            authorization: `token ${token}`,
          },
      ...
      navigation.navigate('PostScreen', {
        post: fetchedPost,
        lecture: comment.course.course_name,
      });
    } catch (e) {
      console.error(e);
    }
          ...
          <TouchableOpacity
            ...
            <View
              ...
              <Text
                ...
                {comment.comment.date.split('T')[0]}
              </Text>
              <Text
                style={{
                  ...GlobalStyles.text,
